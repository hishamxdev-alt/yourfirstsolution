...
if ($image_src && $image_src['error'] === UPLOAD_ERR_OK) {
    ...
    $image_srcPath = "uploads/news/" . $filename;
    move_uploaded_file($image_src['tmp_name'], $uploadPath);
}

try {
    if ($id) {
        $stmt = $pdo->prepare("UPDATE news SET title = ?, sub_title = ?, description = ?" . ($image_srcPath ? ", image_src = ?" : "") . " WHERE id = ?");
        ...
        $stmt = $pdo->prepare("INSERT INTO news (title, sub_title, description, image_src) VALUES (?, ?, ?, ?)");
